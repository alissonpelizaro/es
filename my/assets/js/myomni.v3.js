...
$(document).ready(function(){
  ...
  $("#pendentesBtn").click(function(){
    $.ajax({
      type: 'POST',
      url: '../application/ajaxPendentes',
      data: { local: "lista"},
      success: function(result){
        $("#listaPendentes").html(result);
      }
    });
  });
    ...
    $.ajax({
      ...
      success: function(result){
        if ($("#iconPause").hasClass("fa-pause")) {
          ...
          $("#pauseControl").html("<i id='iconPause' class='fa fa-pause m-r-10'></i>Entrar em pausa");
...
function checaLembreteEstacionados() {
  $.ajax({
    type : "POST",
    url : "../application/checaLembreteEstacionados",
    success : function(result) {
      if (result != "false") {
        var cursor = result.split(";");
        for (var i = 0; i < cursor.length - 1; i++) {
          var temp = cursor[0].split(",");
          var data = temp[0];
          ...
          if (data <= hoje) {
            ...
            setToastInfo('MyOmni', '<a href="media?hash='+hash+'&token='+token+'" >Retorne a conversa com o cliente: ' + nome + '</a>');
          }
        }
      }
    }
  });
  setTimeout(checaLembreteEstacionados, 59000);
}
  ...
  $.ajax({
    ...
    success: function(result){
      if(result != 'false' && result.indexOf("8/8-8/") > 0){
        ...
        document.getElementById('conteudoAlertaBoxBroadcast').innerHTML = nl2br(result[1]);
...
function checkNewMessageAtendimento(){
  ...
  $.ajax({
    ...
    success: function(result){
      ...
      if(result[1] == 'true'){
        if(document.getElementById('notifyNewMessageAtendimentoPendente').style.display == 'none'){
          $('#spanIconNewAtendimento').css('padding-top', '15px');
          $('#notifyNewMessageAtendimentoPendente').show();
          setToastInfo('WhatsApp Chat', 'Um cliente pendente voltou a responder!');
        }
      } else {
        $('#notifyNewMessageAtendimentoPendente').hide();
        /*$('#spanIconNewAtendimento').css('padding-top', '0px');*/
      }
    }
  });
  setTimeout(checkNewMessageAtendimento, 7000);
}
  ...
  $.ajax({
    ...
    success: function(result){
      if(result != '0' && result != ""){
        ...
        setToastDanger('Atenção!', '<a href="remoteChat?hash='+result+'&token=tIxxICb5BQxNe5V">Parece que um atendimento requer a intervenção de um superior!</a>')
      } else {
