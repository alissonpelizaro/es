...
if (!empty($_FILES) && isset($_GET['hash'])) {
  ...
  if($db->query($sql)){
    ...
    if($mediaType != 'invalid'){
      ...
      $response = shell_exec('curl -X POST "'.$url.'" -H "Postman-Token: 68f9212f-b8c3-4648-b23e-b52ad1b3c6c3" -H "auth-key: QjbXR1w2f0bRB55M8jrF2tBU6tpTkkAu" -H "cache-control: no-cache" -H "client_id: t00" -H "content-type: multipart/form-data; boundary=----WebKitFormBoundary7MA4YWxkTrZu0gW" -F "message=@../my/assets/medias/'.$_FILES['file']['name'].';filename='.$_FILES['file']['name'].'" -F message=');
    }
  }
}
