...
class Sincronismo extends Msg {
  ...
  */
  ...
  private function transbordaAtendimento($at){
    ...
    if($agente){
      ...
      $this->transbordaAtendimento($at, $agente);
      ...
      $this->carregaMsg($at['idAtendimento'], $msg, 'cliente', date("Y-m-d H:i:s"));
  ...
  */
  ...
  private function getAgents($fila = false, $plataforma = false){
    ...
    if(is_array($response) && count($response) > 0){
      ...
      foreach ($response as $msg) {
        ...
        if(isset($at['retFirst'])){
          ...
          if(!$this->getUrlImage($fotoCliente, "../my/assets/medias/clients/".$namePic)){
            $namePic = "";
          }
          ...
          if($at == 'Parked'){
            ...
            if($msg->isMedia && $msg->mediaData->type == 'image'){
              ...
              $msg->body = $this->baixaMediaWhatsApp(
                $msg->chat->lastReceivedKey->_serialized,
                $msg->mediaData->type,
            ...
            //Grava a mensagem recebida no chat
            if($this->carregaMsg($at, $this->util->setMessage($msg->body, 'whatsapp'), "cliente", date("Y-m-d H:i:s"))){
              //Mensagem gravada
            } else {
              echo "Erro ao gravar a mensagemddd\n\r";
              // BUG: Erro ao gravar a mensagem
            }
            unset($at);
          }
  ...
  private function gravaCliente($dados){
    $data = date('Y-m-d');
    $arrayTel = json_encode(array(
      array(true, $dados->fone),
      array(false, ''),
      array(false, '')
    ));
    $sql = "INSERT INTO `cliente` (
      ...
      '$dados->nome','$arrayTel','$dados->email','$dados->cpf','$dados->foto', '0', '0', '$data'
    )";
    ...
    if(count($at) > 0){
        ...
        if($checkPark){
          ...
          if(is_numeric($msgIn) && $msgIn > 0){
            ...
            if($fila){
              ...
              $retFirst = "Atendimento iniciado: <b><i class='expi'>".$filaSelecionada."</i> (" . date('d/m/Y H:i:s').").</b>. Protocolo: <b>$protocolo</b>";

              $msg = new Msg();
              $msg->setPlataforma($plataforma);
              $msg->setMsg("Certo! Seu atendimento foi inciado. O número do seu protocolo é: *". $protocolo ."*.");
              $msg->setDst($rmt);
              $msg->sendMessage();
      ...
      if(isset($semFila)){
        ...
        if(isset($transbordo) && $transbordo){
          ...
          $retFirst = "Atendimento iniciado (transbordo): <b><i class='expi'>".$filaSelecionada."</i> (" . date('d/m/Y H:i:s').").</b>. Protocolo: <b>$protocolo</b>";
